...
class MomentumIndicators extends Core
{
    public static function adx(int $startIdx, int $endIdx, array $inHigh, array $inLow, array $inClose, int $optInTimePeriod, int &$outBegIdx, int &$outNBElement, array &$outReal): int
    {
        ...
        $lookbackTotal = 2 * $optInTimePeriod + static::$unstablePeriod[FuncUnstId::ADX->value] - 1;
        ...
        while ($i-- > 0) {
            ...
            $prevClose = $inClose[$today];
            if (! ($prevTR > -0.00000001 && $prevTR < 0.00000001)) {
                $minusDI = 100.0 * ($prevMinusDM / $prevTR);
                $plusDI = 100.0 * ($prevPlusDM / $prevTR);
                $tempReal = $minusDI + $plusDI;
                if (! ($tempReal > -0.00000001 && $tempReal < 0.00000001)) {
                    $sumDX += 100.0 * (abs($minusDI - $plusDI) / $tempReal);
                }
            }
        }
        $prevADX = $sumDX / $optInTimePeriod;
        $i = static::$unstablePeriod[FuncUnstId::ADX->value];
        while ($i-- > 0) {
            $today++;
            $tempReal = $inHigh[$today];
            $diffP = $tempReal - $prevHigh;
    ...
    public static function aroon(int $startIdx, int $endIdx, array $inHigh, array $inLow, int $optInTimePeriod, int &$outBegIdx, int &$outNBElement, array &$outAroonDown, array &$outAroonUp): int
    {
        ...
        $lowest = 0.0;
        $highest = 0.0;
        $factor = 100.0 / (float) $optInTimePeriod;
        while ($today <= $endIdx) {
            $tmp = $inLow[$today];
    ...
    public static function cmo(int $startIdx, int $endIdx, array $inReal, int $optInTimePeriod, int &$outBegIdx, int &$outNBElement, array &$outReal): int
    {
        ...
        $unstablePeriod = static::$unstablePeriod[FuncUnstId::CMO->value];
        if ($unstablePeriod === 0 && static::$compatibility === Compatibility::Metastock->value) {
            ...
            if (! ($tempValue4 > -0.00000001 && $tempValue4 < 0.00000001)) {
        ...
        if ($today > $startIdx) {
            ...
            if (! ($tempValue1 > -0.00000001 && $tempValue1 < 0.00000001)) {
    ...
    public static function dx(int $startIdx, int $endIdx, array $inHigh, array $inLow, array $inClose, int $optInTimePeriod, int &$outBegIdx, int &$outNBElement, array &$outReal): int
    {
        ...
        $lookbackTotal = $optInTimePeriod + static::$unstablePeriod[FuncUnstId::DX->value];
        if ($startIdx < $lookbackTotal) {
            $startIdx = $lookbackTotal;
        }
        if ($startIdx > $endIdx) {
        ...
        $i = static::$unstablePeriod[FuncUnstId::DX->value] + 1;
        while ($i-- > 0) {
            $today++;
            $tempReal = $inHigh[$today];
            $diffP = $tempReal - $prevHigh;
    ...
    public static function mfi(
        ...
        $outBegIdx = 0;
        $outNBElement = 0;
        $lookbackTotal = $optInTimePeriod + static::$unstablePeriod[FuncUnstId::MFI->value];
        if ($startIdx < $lookbackTotal) {
            $startIdx = $lookbackTotal;
        }
        if ($startIdx > $endIdx) {
    ...
    public static function minusDI(int $startIdx, int $endIdx, array $inHigh, array $inLow, array $inClose, int $optInTimePeriod, int &$outBegIdx, int &$outNBElement, array &$outReal): int
    {
        ...
        if ($optInTimePeriod > 1) {
            $lookbackTotal = $optInTimePeriod + static::$unstablePeriod[FuncUnstId::MINUS_DI->value];
        } else {
            $lookbackTotal = 1;
        }
        if ($startIdx < $lookbackTotal) {
        ...
        $i = static::$unstablePeriod[FuncUnstId::MINUS_DI->value] + 1;
        while ($i-- !== 0) {
            $today++;
            $tempReal = $inHigh[$today];
            $diffP = $tempReal - $prevHigh;
    ...
    public static function minusDM(int $startIdx, int $endIdx, array $inHigh, array $inLow, int $optInTimePeriod, int &$outBegIdx, int &$outNBElement, array &$outReal): int
    {
        ...
        if ($optInTimePeriod > 1) {
            $lookbackTotal = $optInTimePeriod + static::$unstablePeriod[FuncUnstId::MINUS_DM->value] - 1;
        } else {
            $lookbackTotal = 1;
        }
        if ($startIdx < $lookbackTotal) {
        ...
        $i = static::$unstablePeriod[FuncUnstId::MINUS_DM->value];
        while ($i-- > 0) {
            $today++;
            $tempReal = $inHigh[$today];
            $diffP = $tempReal - $prevHigh;
    ...
    public static function plusDI(int $startIdx, int $endIdx, array $inHigh, array $inLow, array $inClose, int $optInTimePeriod, int &$outBegIdx, int &$outNBElement, array &$outReal): int
    {
        ...
        if ($optInTimePeriod > 1) {
            $lookbackTotal = $optInTimePeriod + static::$unstablePeriod[FuncUnstId::PLUS_DI->value];
        } else {
            $lookbackTotal = 1;
        }
        if ($startIdx < $lookbackTotal) {
        ...
        $i = static::$unstablePeriod[FuncUnstId::PLUS_DI->value] + 1;
        while ($i-- > 0) {
            $today++;
            $tempReal = $inHigh[$today];
            $diffP = $tempReal - $prevHigh;
    ...
    public static function plusDM(int $startIdx, int $endIdx, array $inHigh, array $inLow, int $optInTimePeriod, int &$outBegIdx, int &$outNBElement, array &$outReal): int
    {
        ...
        if ($optInTimePeriod > 1) {
            $lookbackTotal = $optInTimePeriod + static::$unstablePeriod[FuncUnstId::PLUS_DM->value] - 1;
        } else {
            $lookbackTotal = 1;
        }
        if ($startIdx < $lookbackTotal) {
        ...
        $i = static::$unstablePeriod[FuncUnstId::PLUS_DM->value];
        while ($i-- > 0) {
            $today++;
            $tempReal = $inHigh[$today];
            $diffP = $tempReal - $prevHigh;
    ...
    public static function rsi(int $startIdx, int $endIdx, array $inReal, int $optInTimePeriod, int &$outBegIdx, int &$outNBElement, array &$outReal): int
    {
        ...
        $today = $startIdx - $lookbackTotal;
        $prevValue = $inReal[$today];
        $unstablePeriod = static::$unstablePeriod[FuncUnstId::RSI->value];
        if ($unstablePeriod === 0 &&
    ...
    public static function ultOsc(
        ...
        while ($today <= $endIdx) {
            ...
            $b3Total += $trueRange;
            $output = 0.0;
            if (! ($b1Total > -0.00000001 && $b1Total < 0.00000001)) {
                $output += 4.0 * ($a1Total / $b1Total);
            }
            if (! ($b2Total > -0.00000001 && $b2Total < 0.00000001)) {
                $output += 2.0 * ($a2Total / $b2Total);
            }
            if (! ($b3Total > -0.00000001 && $b3Total < 0.00000001)) {
